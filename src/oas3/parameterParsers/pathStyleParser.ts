...
function parsePathParameter(location: ParameterLocation, value: string, structuredParser: ParameterParser) {
    if(value.startsWith(';')) {
        value = value.slice(1);
    }
    const queryParsedValue = querystring.parse(value, ';', '=', {decodeURIComponent: (val: string) => val});
